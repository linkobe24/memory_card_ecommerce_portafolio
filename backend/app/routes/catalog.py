...
async def search_games(
    ...
    """
    Busca juegos en RAWG aplicando los filtros enviados y devuelve resultados paginados.

    Propaga HTTP 429 cuando RAWG impone rate limit y 503 ante fallos de la API externa.
    """
    try:
...
async def get_game_details(game_id: int, catalog_service: CatalogServiceDep):
    """Obtiene la ficha completa de un juego proveniente de RAWG."""
    try:
...
async def list_genres(catalog_service: CatalogServiceDep):
    """Devuelve el listado de géneros disponibles en RAWG."""
    try:
...
async def list_platforms(catalog_service: CatalogServiceDep):
    """Devuelve todas las plataformas soportadas por RAWG."""
    try:
